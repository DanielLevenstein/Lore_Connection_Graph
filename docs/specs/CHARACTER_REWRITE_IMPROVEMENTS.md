...
Use `Qwen/Qwen2.5-0.5B-Instruct-GGUF` with `Q4_K_M` as the current local model candidate for character summary and backstory rewrites.
...
1. Add a local rewrite model adapter.
   ...
   - Configure `Qwen/Qwen2.5-0.5B-Instruct-GGUF` in code and in the advanced JSON config file, not in editable UI fields.
...
If `Qwen/Qwen2.5-0.5B-Instruct-GGUF` cannot beat the original Orin backstory while preserving required terms after graph-segment prompt fixes, stop and document the failure. The next comparison candidates are TinyLlama and a larger Qwen model, but promotion to a different model should happen in a separate design update rather than silently inside implementation.
